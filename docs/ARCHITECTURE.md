...
PixelHub 是一个基于 Go 语言开发的现代化图床应用，采用三层架构设计，提供 RESTful API 接口。
...
PixelHub 提供标准的 RESTful API，可以轻松与其他服务集成：

1. **AI 服务集成**
   - 通过 API 接口实现图片检索
   - 支持第三方协议（如 MCP）的独立适配层
...
**推荐架构**：
- 将特定协议适配层（如 MCP Server）独立部署
- 通过 API 与 PixelHub 后端通信
- 保持核心服务的简洁和专注
